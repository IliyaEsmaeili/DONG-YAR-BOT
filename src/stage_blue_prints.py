...
async def dong_creation_router(message):
    ...
    match stage:
        case "stage_begin":
            await stage_begin(message, user)
        case "stage_name":
            await stage_name(message, user)
        case "stage_amount":
            await stage_amount(message, user)
        case "stage_participants":
            await stage_participants(message, user)
        case "stage_additional_info":
            await stage_additional_info(message, user)
        case "stage_confirm":
...
async def stage_participants(message, user):
    ...
    await execute_query("""DELETE
                           ...
                           WHERE dong_id = $1""", user.dong[-1].local_dong_id)
    for participant in participants:
        await execute_query("""INSERT INTO dong_participants(dong_id, user_name)
                               VALUES ($1, $2)
